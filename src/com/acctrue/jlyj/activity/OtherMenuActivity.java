		...
		if (fileList.length==1) {
			...
			for (int i = 0; i < fileList.length; i++) {
				...
				if (!file.equals("sdcard1")) {
					...
					File producedataDir = new File(uPadFilePath+"/药品监管_出入库台帐_文件");
...
//					if(commonService.cropType == 0){
						HttpGet request0 = new HttpGet(commonService.serverUrl
								+ "/Portal/ScanServices/CodeService.svc/DownExcel/0");
						HttpResponse response0 = new DefaultHttpClient().execute(request0);
						int connectStatus0 = response0.getStatusLine().getStatusCode();
						if (connectStatus0 == HttpStatus.SC_OK) {
							//判断如果有旧模版则删除

							File modeProduceFile = new File(theProduceDataPath1+"/生产批次模版.xls");
							
							if (modeProduceFile.exists()) {
								modeProduceFile.delete();
							}
													
							byte[] buffer = EntityUtils.toByteArray(response0.getEntity());						
							FileOutputStream fout = new FileOutputStream(theProduceDataPath1+"/生产批次模版.xls");  
							fout.write(buffer);   					   
							fout.close();
...
//					}
					...
					if (connectStatus == HttpStatus.SC_OK) {
						...
						File modeIODataFile = new File(theProduceDataPath1+"/出入库台帐模版.xls");
						
						if (modeIODataFile.exists()) {
							modeIODataFile.delete();
						}					
						byte[] buffer = EntityUtils.toByteArray(response.getEntity());						
						FileOutputStream fout = new FileOutputStream(theProduceDataPath1+"/出入库台帐模版.xls");  
						fout.write(buffer);   					   
						fout.close();  
		...
		new Thread(new Runnable() {
			...
			// TODO Auto-generated method stub
				for (int i = 0; i < theProduceDataFileList.length; i++) {
					...
					//文件读取
					try {
						FileInputStream fin = new FileInputStream(theProduceDataPath+"/"+theProduceDataFileList[i]);
						...
						//上传
						HttpPost request = new HttpPost(commonService.serverUrl
								+ "/Portal/ScanServices/CodeService.svc/UploadExcel/1/"+fileNamePre+"/"+commonService.cropCode);
						...
						if (connectStatus == HttpStatus.SC_OK) {
							...
							File file = new File(theProduceDataPath+"/"+theProduceDataFileList[i]);
							if (errMsg.equals("null")) {
								
								String byteStr = object.getString("Bytes");
								
								byte[] returnBuffer = Base64.decode(byteStr, Base64.DEFAULT);
								
								if (byteStr.equals("null")) {
									file.renameTo(new File(theProduceDataPath+"/"+fileNamePre+"_上传成功"+"."+fileNameLast));
								}else{
									file.delete();

									File errFile = new File(theProduceDataPath+"/"+fileNamePre+"_部分上传失败"+"."+fileNameLast);
									
									FileOutputStream fOut = new FileOutputStream(errFile);
									...
									file.renameTo(new File(theProduceDataPath+"/"+fileNamePre+"_"+errMsg+"."+fileNameLast));
	...
	public void uploadProduceData()
	{
		...
		if (theProduceDataFileList.length==0) {
			Toast.makeText(this, "药品监管_生产批次_文件 中没有待上传文件", Toast.LENGTH_LONG).show();
			return;
		}
		dialog = new ProgressDialog(this);
		dialog.setProgressStyle(ProgressDialog.STYLE_HORIZONTAL);// 设置水平进度条  
			...
			// TODO Auto-generated method stub
				for (int i = 0; i < theProduceDataFileList.length; i++) {
					...
					try {
						...
						//上传
						HttpPost request = new HttpPost(commonService.serverUrl
								+ "/Portal/ScanServices/CodeService.svc/UploadExcel/0/"+fileNamePre+"/"+commonService.cropCode);
						ByteArrayEntity entity=new ByteArrayEntity(buffer);
						entity.setContentType("application/x-www-form-urlencoded");
						request.setEntity(entity);
						HttpResponse response = new DefaultHttpClient().execute(request);
